...
def setZone (){
//	sendEvent(name: "zone", value: Zone, isStateChange: true)
//	state.zone = Zone
//	log.debug device.deviceNetworkId
	len = device.deviceNetworkId.length()
	state.zone = device.deviceNetworkId.substring(len-2,len).toInteger()
//	log.debug "Reading the zone form internal variable "+ state.zone
...
def nextTrack(){
	if(state.mediaSource<6)
	{
		...
		if (logEnable) {
			...
			parent.sendMsg ("Zone${state.zone}ch=0${state.mediaSource}")
  			} 
			catch (Exception e) {
...
def setLevel (Value){
    ...
    try {
	    ...
	    parent.sendMsg ("Zone${state.zone}vo=${state.volume}")
  } catch (Exception e) {
...
def on() {
	...
	try {
		state.switch = "on"
        	sendEvent(name: "switch", value: "on", isStateChange: true)
		parent.sendMsg ("Zone${state.zone}pr=01")
        }
	 catch (Exception e) {
...
def off() {
    ...
    try {
	 	state.switch = "off"
        	sendEvent(name: "switch", value: "off", isStateChange: true)
		parent.sendMsg ("Zone${state.zone}pr=00")
        }
	catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
...
def mute() {
    ...
    try {
	    	state.mute = "muted"
		sendEvent(name: "mute", value: "muted", isStateChange: true)
	    	parent.sendMsg ("Zone${state.zone}mu=01")
  } catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
}
def unmute() {
    ...
    try {
	    	state.mute = "unmuted"
		sendEvent(name: "mute", value: "unmuted", isStateChange: true)
	    	parent.sendMsg ("Zone${state.zone}mu=00")
  } catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
}
